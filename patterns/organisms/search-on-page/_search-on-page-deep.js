 ...
 * SearchOnPage
 */
export default class SearchOnPageDeep {
  ...
  constructor() {
    ...
    if (this.form.element.length) {
      ...
      this.searchTerms = $('[data-role="sop-term-section"]').map(function () {
        let termSection = $(this);
        let groups = $('[data-role="sop-term-group"]').map(function () {
          let termGroup = $(this);
  ...
  filterResults(searchPhrase) {
    ...
    for (let k = 0; k < this.searchTerms.length; k++) {
      let isFoundSection = false;
      for (let i = 0; i < this.searchTerms[k].groups.length; i++) {
        let isFound = false;
        for (let j = 0; j < this.searchTerms[k].groups[i].terms.length; j++) {
          this.searchTerms[k].groups[i].terms[j].isFound =
            this.searchTerms[k].groups[i].terms[j].title.indexOf(searchPhrase) !== -1 ||
            this.searchTerms[k].groups[i].terms[j].description.indexOf(searchPhrase) !== -1;
          if (this.searchTerms[k].groups[i].terms[j].isFound) isFound = true;
        }
        this.searchTerms[k].groups[i].isFound = isFound;
        if (this.searchTerms[k].groups[i].isFound) isFoundSection = true;
      }
      this.searchTerms[k].isFound = isFoundSection;
    }
  }

  resetResults() {
    for (let k = 0; k < this.searchTerms.length; k++) {
      this.searchTerms[k].isFound = true;
      for (let i = 0; i < this.searchTerms[k].groups.length; i++) {
        this.searchTerms[k].groups[i].isFound = true;
        for (let j = 0; j < this.searchTerms[k].groups[i].terms.length; j++) {
          this.searchTerms[k].groups[i].terms[j].isFound = true;
        }
      }
    }
  }
  ...
  displayResults() {
    let foundQty = 0;
    for (let k = 0; k < this.searchTerms.length; k++) {
      if (this.searchTerms[k].isFound) {
        this.searchTerms[k].element.show();
        for (let i = 0; i < this.searchTerms[k].groups.length; i++) {
          if (this.searchTerms[k].groups[i].isFound) {
            this.searchTerms[k].groups[i].element.show();
            for (let j = 0; j < this.searchTerms[k].groups[i].terms.length; j++) {
              if (this.searchTerms[k].groups[i].terms[j].isFound) {
                this.searchTerms[k].groups[i].terms[j].element.show();
                foundQty++;
              } else {
                this.searchTerms[k].groups[i].terms[j].element.hide();
              }
            }
          } else {
            this.searchTerms[k].groups[i].element.hide();
          }
        }
      } else {
        this.searchTerms[k].element.hide();
      }
    }
